...
export default class Navigation {
    ...
    _getParams() {
        ...
        const paramsColon = params[params.length - 1].indexOf(":") > -1
            ? params[params.length - 1]
            : undefined;
        const paramsEqual = params[params.length - 1].indexOf("=") > -1
            ? params[params.length - 1]
            : undefined;
        ...
        return [
            {
                icon: "map",
                type: "link",
                txt: `<a href="${iff(params, `#/city/${city}/${params}`, `#/city/${city}/`)}">
            <div class="open_div">
              <span
                class="${iff(hash.indexOf("city") > -1 && hash.indexOf("tracks") === -1, "icon iconmap nav-icon active", "icon iconmap nav-icon")}"></span>
            </div>
                </a >`
            },
            {
                icon: "song",
                type: "link",
                txt: `<a href="${iff(this._getParams(), `#/city/${city}/tracks/${params}/`, `#/city/${city}/tracks/`)}">
            <div class="open_div">
              <span
                class="${iff(hash.indexOf("tracks") > -1, "icon iconsong nav-icon active", "icon iconsong nav-icon")}"></span>
            </div>
                </a >`
            },
            {
            ...
            <div class="open_div">
              <span
                class="${iff(hash.indexOf("favorites") > -1, "icon iconfavorite nav-icon active", "icon iconfavorite nav-icon")}"></span>
            </div>
                </a >`
            },
            {
