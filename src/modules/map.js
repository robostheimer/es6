...
export default class Map {
    fetchGeolocation() {
        return geolocation.getGeolocation().then((options) => {
            if (options) {
                return this.fetchTopTracksFromLocation(`${options.latitude},${options.longitude}`).then(data => {
            ...
            return this.fetchTopTracksFromLocation(`${position.position.latitude},${position.position.longitude}`).then(data => {
                const mapMarkers = this.sortMarkers(data);
                const options = { tag: "#container", data };
                return options;
            });
