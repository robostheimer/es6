...
export default class FavoriteButton {
    ...
    toggleFavorite(options) {
        ...
        let favorites;
        if (localStorage.getItem("FavoriteArr") != null &&
        ...
        const isFavorited = dom.getAttribute("class").indexOf("iconfavorite2") === -1;
        if (isFavorited) {
            dom.classList.add("iconfavorite2");
            dom.classList.remove("iconfavorite");
            localStorage.setItem("FavoriteArr", JSON.stringify(removeItem(favorites, item, "id")));
        }
        else {
